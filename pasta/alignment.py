...
class Alignment(dict, object):
    ...
    def write_filepath(self, filename, file_format='FASTA', zipout=False):
        ...
        if not using_spark:
            ...
            assert file_format.upper() == 'FASTA'
            self.write(filename, file_format=file_format + '-SPARK')
        ...
        for line_index, line in enumerate(stream):
            ...
            if s.startswith('>'):
                ...
                if curr_taxon in self.char_matrix:
                    raise DataParseError(message="Fasta error: Repeated sequence name (%s) found" % name,
                                         row=line_index + 1, stream=stream)
                if curr_vec is not None and len(curr_vec) == 0:
                    raise DataParseError(
                        message="Fasta error: Expected sequence, but found another sequence name (%s)" % name,
                        row=line_index + 1, stream=stream)
                if self.simple_rows:
                    curr_vec = []
                else:
                ...
                raise DataParseError(message="Fasta error: Expecting a lines starting with > before sequences",
                                     row=line_index + 1, stream=stream)
            else:
                if self.simple_rows:
                    ...
                    if m:
                        raise DataParseError(
                            message='Unrecognized sequence symbol "%s" (check to make sure the --datatype is set properly)' % m.group(
                                0), row=line_index + 1, column=m.start(), stream=stream)
                    curr_vec.append(s)
                else:
                    for col_ind, c in enumerate(s):
                        ...
                        try:
                            ...
                            raise DataParseError(message='Unrecognized sequence symbol "%s"' % c, row=line_index + 1,
                                                 column=col_ind + 1, stream=stream)
        if self.simple_rows and curr_taxon and curr_vec:
            self.char_matrix[curr_taxon] = "".join(curr_vec)
...
class MultiLocusDataset(list):
    ...
    def create_dendropy_dataset(self):
        ...
        for n, element in enumerate(self):
            if not isinstance(element, SequenceDataset):
                raise ValueError(
                    "Expecting all elements of MultiLocusDataset to be SequenceDataset objects when create_dataset is called!")
    ...
    def relabel_for_pasta(self):
        ...
        for n, element in enumerate(self):
            if not isinstance(element, SequenceDataset):
                raise ValueError(
                    "Expecting all elements of MultiLocusDataset to be SequenceDataset objects when relabel_for_pasta is called!")
            try:
                taxa_block = element.taxa
